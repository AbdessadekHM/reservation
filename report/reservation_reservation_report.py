...
class ReservationReport(models.Model):
    ...
    partner_id = fields.Many2one(
        "res.partner", "Client", aggregator="sum", readonly=True
    )
    reservation_start_date = fields.Date(
        default=fields.Date.today(), string="Start date", store=False, readonly=True
    )
    reservation_end_date = fields.Date(
        default=fields.Date.today(), string="End date", store=False, readonly=True
    )
